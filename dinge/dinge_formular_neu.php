...
<table>
<tr><td><textarea name="beschreibung_dinge" cols="41" rows="5"> <?php echo trim($data['beschreibung_dinge']) ?></textarea></td></tr>
</table>
<br>
...
<?php
$erg = $pdo->prepare("SELECT id_ort, name_ort, fs_regal FROM tab_ort"); 
$result = $erg->execute();
	while($data = $erg->fetch()) {
		$id=$data['id'];
    // echo "<option value =".$id.'>'. $data['name_regal']."</option>";
...
<tr></tr>
<tr>
 <td><input type="Submit" name="" formaction="dinge_speichern.php" value="speichern"></td>
