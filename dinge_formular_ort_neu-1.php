...
// hier wird die Dropdownliste aus der DB erstellt
...
$result = $erg->execute();
	...
	while($data = $erg->fetch()) {
		$id=$data['id'];
    // echo '<td><a href=dinge_formular.php?ID='.$id.'>'. $data['id_stockwerk'] . '</a></td>';
    // echo '<td><a href=dinge_formular.php?ID='.$id.'>'. $data['name_zimmer'] . '</a></td>';
...
<!-- hier beginnt das alte Formular -->
